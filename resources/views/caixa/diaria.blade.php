...
{{-- Aviso de comandas abertas --}}
<div class="panel mb-5" style="border-color:{{ $comandasAbertas->isEmpty() ? 'rgba(34,197,94,.25)' : 'rgba(249,115,22,.35)' }}">
  <div class="panel-header">
    <div class="panel-title">{{ $comandasAbertas->isEmpty() ? '✅ Nenhuma comanda aberta' : '⚠️ Comandas abertas antes do fechamento' }}</div>
    <span class="badge {{ $comandasAbertas->isEmpty() ? 'badge-success' : 'badge-warning' }}">{{ $comandasAbertas->count() }}</span>
  </div>
  @if($comandasAbertas->isNotEmpty())
  <div style="font-size:13px;color:var(--muted);margin-bottom:12px">
    Valor pendente total: <strong style="color:#fff">R$ {{ number_format($valorPendente,2,',','.') }}</strong>
  </div>
  <div style="overflow:auto">
    <table style="width:100%">
      <thead>
        <tr>
          <th style="text-align:left;font-size:11px;color:var(--muted);padding:6px">Comanda</th>
          <th style="text-align:left;font-size:11px;color:var(--muted);padding:6px">Mesa</th>
          <th style="text-align:left;font-size:11px;color:var(--muted);padding:6px">Garçom</th>
          <th style="text-align:left;font-size:11px;color:var(--muted);padding:6px">Status</th>
          <th style="text-align:right;font-size:11px;color:var(--muted);padding:6px">Pendente</th>
        </tr>
      </thead>
      <tbody>
        @foreach($comandasAbertas as $comanda)
        <tr style="border-top:1px solid var(--border)">
          <td class="td-mono" style="padding:8px;color:var(--accent)">#{{ str_pad($comanda->id,4,'0',STR_PAD_LEFT) }}</td>
          <td style="padding:8px">Mesa {{ $comanda->table->numero ?? '—' }}</td>
          <td style="padding:8px;color:var(--muted)">{{ $comanda->user->name ?? '—' }}</td>
          <td style="padding:8px"><span class="badge badge-warning">{{ str_replace('_',' ',ucfirst($comanda->status)) }}</span></td>
          <td style="padding:8px;text-align:right;font-weight:800;color:#fff">R$ {{ number_format($comanda->total,2,',','.') }}</td>
        </tr>
        @endforeach
      </tbody>
    </table>
  ...
  @else
  <div style="font-size:13px;color:var(--muted)">Não há mesa/comanda pendente para este fechamento.</div>
  @endif
</div>
  ...
  <div class="panel">
    ...
    <div style="margin-top:12px;display:grid;grid-template-columns:repeat(auto-fit,minmax(170px,1fr));gap:10px;">
      <div style="padding:12px;border:1px solid var(--border);border-radius:10px;">
        <div style="font-size:11px;color:var(--muted);text-transform:uppercase">Dinheiro esperado</div>
        <div style="font-weight:900;color:#facc15">R$ {{ number_format($resumoPeriodo['dinheiro'],2,',','.') }}</div>
      </div>
      <div style="padding:12px;border:1px solid var(--border);border-radius:10px;">
        <div style="font-size:11px;color:var(--muted);text-transform:uppercase">Compras + sangrias</div>
        <div style="font-weight:900;color:#f87171">R$ {{ number_format($resumoPeriodo['compras'] + $resumoPeriodo['sangrias'],2,',','.') }}</div>
      </div>
      <div style="padding:12px;border:1px solid rgba(34,197,94,.25);border-radius:10px;">
        <div style="font-size:11px;color:var(--muted);text-transform:uppercase">Valor esperado em caixa</div>
        <div style="font-weight:900;color:#4ade80">R$ {{ number_format($resumoPeriodo['valor_esperado_caixa'],2,',','.') }}</div>
      </div>
    </div>
